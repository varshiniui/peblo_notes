...
const mapNoteWithTags = (note: any) => ({
  ...note,
  tags: note.tags?.map((noteTag: any) => noteTag.tag) ?? [],
});
...
router.patch('/:id', authenticateToken, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const normalizedTagNames = Array.from(new Set(
      tags
        .map((tagName: unknown) => typeof tagName === 'string' ? tagName.trim() : '')
...
router.patch('/:id/pin', authenticateToken, async (req: AuthRequest, res: Response) => {
  try {
    const userId = req.user?.userId;
    const id = req.params['id'] as string;
...
router.delete('/:id/share', authenticateToken, async (req: AuthRequest, res: Response) => {
  try {
    const userId = req.user?.userId;
    const noteId = req.params['id'] as string;
